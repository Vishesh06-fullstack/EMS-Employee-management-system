...
import { dummyEmployeeDashboardData, dummyProfileData } from "../assets/assets";
...
const Sidebar = () => {
  ...
  const sidebarContent = (
    <>
      {/* band header */}
      <div className="px-6 pt-6 pb-5 border-b boder-white/6">
      ...
      <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300 " : ""} `}>
              {isActive && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-indigo-500" />
              )}
              <item.icon
                className={`w-[17px] h-[17px] shrink-0 ${isActive ? "text-indigo-500" : ""}`}
              />
      ...
      <div className="p-3 border-t border-white/6">
          <button className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-500 hover:bg-rose-500/8 transition-all duration-150">
            <LogOutIcon onClick={handleLogout} className="w-[17px] h-[17px] "/>Logout
          </button>
      </div>
    </>
  );
  //closing mobile sidebar on route change
  useEffect(() => {
    setMobileOpen(false);
  }, [pathname]);
